...
| **File uploads — attachments** | Uploaded attachments are validated by file extension (allowlist: `.pdf`, `.xlsx`, `.xls`, `.csv`, `.txt`, `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.docx`, `.doc`, `.zip`). File size is capped at **50 MB**. Files are stored with a random UUID filename. |
